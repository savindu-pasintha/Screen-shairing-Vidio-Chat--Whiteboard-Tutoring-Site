									...
									onmessage: function (msg, jsep) {
										...
										if (event) {
											if (event === "joined") {
												...
												$('#list').empty();
												if (msg["participants"]) {
													var listMSG = msg["participants"];
													Janus.debug("Got a list of participants:", listMSG);
													for (var fMSG in listMSG) {
														var idMSG = listMSG[fMSG]["id"];
														var displayMSG = listMSG[fMSG]["display"];
														var setupMSG = listMSG[fMSG]["setup"];
														var mutedMSG = listMSG[fMSG]["muted"];
														var spatialMSG = listMSG[fMSG]["spatial_position"];
														Janus.debug("  >> [" + idMSG + "] " + displayMSG + " (setup=" + setupMSG + ", muted=" + mutedMSG + ")");
														if ($('#rp' + id).length === 0) {
															// Add to the participants list
															var sliderMSG = '';
															if (spatialMSG !== null && spatialMSG !== undefined)
																slider = '<span>[L <input id="sp' + idMSG + '" type="text" style="width: 10%;"/> R] </span>';
															$('#list').append('<li id="rp' + idMSG + '" class="list-group-item">' +
																sliderMSG +
																displayMSG +
																' <i class="absetup fa fa-chain-broken"></i>' +
																' <i class="abmuted fa fa-microphone-slash"></i></li>');
															if (spatialMSG !== null && spatialMSG !== undefined) {
																$('#sp' + idMSG).slider({ min: 0, max: 100, step: 1, value: 50, handle: 'triangle', enabled: false });
																$('#position').removeClass('hide').show();
															}
															$('#rp' + idMSG + ' > i').hide();
														}
														if (mutedMSG === true || mutedMSG === "true")
															$('#rp' + idMSG + ' > i.abmuted').removeClass('hide').show();
														else
															$('#rp' + idMSG + ' > i.abmuted').hide();
														if (setupMSG === true || setupMSG === "true")
															$('#rp' + idMSG + ' > i.absetup').hide();
														else
															$('#rp' + idMSG + ' > i.absetup').removeClass('hide').show();
														if (spatialMSG !== null && spatialMSG !== undefined)
															$('#sp' + idMSG).slider('setValue', spatialMSG);
													}
												}
